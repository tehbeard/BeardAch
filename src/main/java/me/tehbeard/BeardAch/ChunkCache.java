		...
		if(cache.containsKey(""+world+","+cx+","+cz)){
			...
			for(Achievement a : cache.get(""+world+","+cx+","+cz)){
				...
				if(a.checkAchievement(player)){
					BeardAch.self.getAchievementManager().playerCheckCache.get(a).remove(player.getName());
					BeardAch.self.getAchievementManager().playerHasCache.get(player.getName()).add(new AchievementPlayerLink(a.getSlug()));
					BeardAch.self.getAchievementManager().database.setPlayersAchievements(player.getName(),a.getSlug());
				}
